...
cpu_list=['clock','cores','enabled_cores','frequency','physid','product','threads','vendor','version']
...
network_list=['driver','firmware','serial','link','product','rx-vlan-offload','tx-vlan-offload','vendor']
memory_list=['clock','description','product','size','slot','vendor']
disk_list=['model','size','vendor']
...
def extra_options_print(input_main_key,input_sub_key):
    if input_sub_key == 'cpu':
        ...
        for keys in data[input_main_key][input_sub_key].keys():
            x.add_row([data[input_main_key][input_sub_key][keys][eachparameter]
                      if data[input_main_key][input_sub_key][keys][eachparameter] else 'None'
                      for eachparameter in network_list])
        print(x.get_string())
        ...
        for keys in data[input_main_key][input_sub_key].keys():
            if not keys.startswith('logical'):
                x.add_row([data[input_main_key][input_sub_key][keys][eachparameter]
                           for eachparameter in disk_list])
        print(x.get_string())
    elif input_sub_key == 'firmware':
        x.field_names=data[input_main_key][input_sub_key]['bios'].keys()
        x.add_row([eachvalue for eachvalue in
                   data[input_main_key][input_sub_key]['bios'].values()])
        print(x.get_string())
    elif input_sub_key == 'system':
        x.field_names=data[input_main_key][input_sub_key]['kernel'].keys()
        x.add_row([eachvalue for eachvalue in
